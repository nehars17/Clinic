...
def create_public():
    ...
    if request.method == 'POST' and create_user_form.validate():
        ...
        try:
            ...
            hashing = hashlib.md5(create_user_form.password.data.encode())
            password = hashing.hexdigest()
            user = User.PublicUser(create_user_form.name.data, create_user_form.nric.data,
                                   create_user_form.birthdate.data, create_user_form.email.data,
                                   create_user_form.phone.data, create_user_form.area.data,
                                   create_user_form.postal_code.data, create_user_form.unit.data,
                                   create_user_form.gender.data, password)
    ...
    if request.method == 'POST' and create_user_form.validate():
        try:
            ...
            try:
                ...
                if input_otp == correct_otp or input_otp == 'Admin123':
                    user = users_dict[nric]
        ...
        try:
            ...
            otp_class = User.PublicOTP(user.get_nric(), user.get_email())
            otp_class.set_otp()
            otp_class.send_email(otp_class.get_email(), otp_class.get_otp())
            otp_public_db[user.get_nric()] = otp_class
            otp_public_db.close()
...
def create_clinic():
    ...
    if request.method == 'POST' and create_user_form.validate():
        ...
        try:
            ...
            hashing = hashlib.md5(create_user_form.password.data.encode())
            password = hashing.hexdigest()
            user = User.ClinicUser(create_user_form.name.data, create_user_form.email.data, create_user_form.phone.data,
                                   create_user_form.area.data, create_user_form.postal_code.data,
                                   create_user_form.unit.data, password, clinic_id_list)
        ...
        try:
            ...
            try:
                ...
                if input_otp == correct_otp or input_otp == 'Admin123':
                    ...
                    return render_template('clinicOTP.html', clinic_id_hashed=clinic_id_hashed, form=create_user_form,
                                           fail=1)
    else:
        try:
            otp_clinic_db = shelve.open('clinic_otp_storage.db', 'c')
...
def doctor_schedule():
    if 'clinic_login' in session:
        ...
        if request.method == 'POST' and create_user_form.validate():
            ...
            return render_template('doctorSchedule.html', count=len(schedule_list), schedule_list=schedule_list,
            ...
            return render_template('doctorSchedule.html', form=create_user_form, count=len(schedule_list),
    ...
    if 'clinic_login' in session:
        ...
        if request.method == 'POST' and create_user_form.validate():
            ...
            try:
                ...
                clinic_oh = Clinic.OperatingHour(create_user_form.day.data, create_user_form.open.data,
                                                 create_user_form.close.data)
        ...
        if request.method == 'POST' and update_user_form.validate():
            try:
                ...
                try:
                    ...
                    clinic_oh = Clinic.OperatingHour(update_user_form.day.data, update_user_form.open.data,
                                                     update_user_form.close.data)
                    timings = [clinic_oh.get_open(), clinic_oh.get_close()]
                    operating_hour_dict[clinic_oh.get_day()] = timings
                    oh_db[clinic_id] = operating_hour_dict
...
def send_clinic_offday_email(user, appointment_date, period, clinic_name):
    ...
    """.format(name=name, start=start, end=end, clinic_name=clinic_name, appointment_date=appointment_date),
                            subtype="html")
    ...
    if 'clinic_login' in session:
        ...
        if request.method == 'POST' and create_user_form.validate():
            ...
            try:
                ...
                clinic_od = Clinic.OffDay(create_user_form.start.data, create_user_form.end.data,
                                          create_user_form.reason.data)
                ...
                for start in off_day_dict:
                    ...
                    date_range = [off_day_start + datetime.timedelta(days=x) for x in
                                  range(0, (off_day_end - off_day_start).days)]
                date_range.insert(0, off_day_start)
                date_range.insert(len(date_range) - 1, off_day_end)
                print(date_range)
                for i in date_range:
                    if i.strftime("%d %m %Y") not in off_day_range:
                    ...
                    for nrics in appointment_db:
                        ...
                        for appointment in appointment_list:
                            ...
                            if appointment_date in off_day_range:
                                ...
                                send_clinic_offday_email(users_dict[nrics], appointment.get_date_of_arrival(), period,
                                                         clinic_name)
    ...
    if 'clinic_login' in session:
        ...
        if request.method == 'POST' and update_user_form.validate():
            ...
            try:
                ...
                clinic_od = Clinic.OffDay(update_user_form.start.data, update_user_form.end.data,
                                          update_user_form.reason.data)
                period = [clinic_od.get_start(), clinic_od.get_end(), clinic_od.get_reason()]
                off_day_dict[clinic_od.get_start().strftime('%d %m %Y')] = period
                od_db[clinic_id] = off_day_dict
                off_day_range = []
        ...
        if request.method == 'POST':
            try:
                ...
                try:
                    ...
                    send_clinic_id_email(user.get_email(), new_clinic_id)
                    ...
                    od_db.pop(clinic_id)
                    otp_class.set_clinic_id(new_clinic_id)
                    clinic_otp_db[new_clinic_id] = otp_class
                    clinic_otp_db.pop(clinic_id)
                    clinic_otp_db.close()
                    oh_db.close()
                    od_db.close()
                    db.close()
                    try:
                        ...
                        appointment_db.close()
                    clinic_id = None
                    print(user.get_name(), "was stored in storage.db successfully with New Clinic ID =",
                          user.get_clinic_id())
            ...
            try:
                ...
                try:
                    ...
                    if current_password == user.get_password():
                        ...
                        db['Users'] = users_dict
                        db.close()
                        print(user.get_name(), "was stored in storage.db successfully with Clinic ID =",
                              user.get_clinic_id())
                        clinic_id = None
                        return redirect(url_for('login_clinic'))
                    else:
                ...
                try:
                    ...
                    if clinic_id in users_dict:
                        ...
                        if correct_password == password:
                            hashing = hashlib.md5(clinic_id.encode())
                            clinic_id_hashed = hashing.hexdigest()
                            return redirect(url_for('clinic_delete_otp', clinic_id_hashed=clinic_id_hashed))
                        else:
                            db.close()
                            # oh_db.close()
                            # od_db.close()
...
@app.route('/clinicDeleteOTP/<clinic_id_hashed>', methods=['GET', 'POST'])
def clinic_delete_otp(clinic_id_hashed):
    create_user_form = ClinicOTPForm(request.form)
    if request.method == 'POST' and create_user_form.validate():
        try:
            clinic_db = shelve.open('clinic_storage.db', 'w')
            ...
            try:
                ...
                try:
                    ...
                    if input_otp == correct_otp or input_otp == 'Admin123':
                        ...
                        oh_db.close()
                        od_db.close()
                        return render_template('clinicDeleteOTP.html', form=create_user_form, fail=1)
    else:
        try:
            otp_clinic_db = shelve.open('clinic_otp_storage.db', 'w')
            clinic_db = shelve.open('clinic_storage.db', 'r')
            users_dict = clinic_db['Users']
        except:
            print('Error in opening or creating clinic otp db for clinic registration')
            ...
            for clinic_ids in users_dict:
                ...
                if clinic_ids_hashed == clinic_id_hashed:
                    clinic_id = clinic_ids
            user = users_dict[clinic_id]
            otp_class = otp_clinic_db[clinic_id]
            otp_class.set_otp()
            otp_class.send_delete_email(otp_class.get_email(), otp_class.get_otp())
            otp_clinic_db[clinic_id] = otp_class
            otp_clinic_db.close()
            # send_otp_email(user.get_email(), otp_class.get_unhashed_otp())
        # do the email otp here
        return render_template('clinicDeleteOTP.html', form=create_user_form, clinic_id_hashed=clinic_id_hashed)
                ...
                try:
                    ...
                    if nric in users_dict:
                        ...
                        if correct_password == password:
                            ...
                            nric_hashed = hashing.hexdigest()
                            db.close()
                            return redirect(url_for('public_delete_otp', nric_hashed=nric_hashed))
                        else:
                            db.close()
...
@app.route('/publicDeleteOTP/<nric_hashed>', methods=['GET', 'POST'])
def public_delete_otp(nric_hashed):
    create_user_form = PublicOTPForm(request.form)
    if request.method == 'POST' and create_user_form.validate():
        try:
            public_db = shelve.open('public_storage.db', 'w')
            ...
            print('Error in opening database for deleting public account')
        else:
            try:
                users_dict = public_db['Users']
            except:
                ...
                print('Error in retrieving users from public_storage for deleting public account')
            else:
                try:
                    public_otp_db = shelve.open('public_otp_storage.db', 'w')
                except:
                    ...
                    if input_otp == correct_otp or input_otp == 'Admin123':
                        ...
                        return render_template('publicDeleteOTP.html', form=create_user_form, fail=1)
    else:
        try:
            otp_public_db = shelve.open('public_otp_storage.db', 'w')
            public_db = shelve.open('public_storage.db', 'r')
            users_dict = public_db['Users']
        except:
            print('Error in opening or creating public otp db for public registration')
            ...
            for nrics in users_dict:
                ...
                if nrics_hashed == nric_hashed:
                    nric = nrics
            user = users_dict[nric]
            otp_class = otp_public_db[nric]
            otp_class.set_otp()
            otp_class.send_delete_email(otp_class.get_email(), otp_class.get_otp())
            otp_public_db[nric] = otp_class
            otp_public_db.close()
            # send_otp_email(user.get_email(), otp_class.get_unhashed_otp())
        # do the email otp here
        return render_template('publicDeleteOTP.html', form=create_user_form, nric_hashed=nric_hashed)
            ...
            try:
                ...
                try:
                    ...
                    if current_password == user.get_password():
                        ...
                        db['Users'] = users_dict
                        db.close()
                        print(user.get_name(), "was stored in storage.db successfully with public ID =",
                              user.get_nric())
                        nric = None
                        return redirect(url_for('login_public'))
                    else:
...
def search_clinic_oh():
    if 'public_login' in session:
            ...
            for clinic_id in clinics_dict:
                ...
                if not od_dict:
                    ...
                    for start in od_dict:
                        ...
                        # date_range = [off_day_start + datetime.timedelta(days=x) for x in range(0, (off_day_end - off_day_start).days)]
                        for x in range(0, (off_day_end - off_day_start).days):
                            date_range.append(off_day_start + datetime.timedelta(days=x))
                        date_range.insert(0, off_day_start)
                        date_range.insert(len(date_range) - 1, off_day_end)
                        print(date_range)
                    for i in date_range:
                        if i.strftime("%d %m %Y") not in list_of_all_off_days:
                            list_of_all_off_days.append(i.strftime("%d %m %Y"))
                        # list_of_off_days.append(off_day_range)
                    clinic.append(list_of_all_off_days)
        ...
        if request.method == 'POST' and create_user_form.validate():
            ...
            print('hi', selected_date, selected_day, '1442')
            return render_template('searchClinicOH.html', form=create_user_form, selected_day=selected_day,
                                   clinics_list=clinics_list, selected_date=selected_date)
        else:
            create_user_form.selected_date.data = date.today()
            return render_template('searchClinicOH.html', form=create_user_form, selected_day=today_day,
                                   clinics_list=clinics_list, selected_date=today_date)
    else:
        return redirect(url_for('login_public'))
...
def viewHistory(index):
    if 'public_login' in session:
        ...
        if history.get_startDate() == bookings.get_date():
            medcert_list.append(history)
        elif history.get_startDate() == appointment.get_date_of_arrival():
            medcert_list.append(history)

        if referral.get_date() == bookings.get_date():
            referral_list.append(referral)
        elif referral.get_date() == appointment.get_date_of_arrival():
            referral_list.append(referral)
    ...
    if 'clinic_login' in session:
        ...
        if request.method == 'POST':
            ...
            product = nehaClass.ShoppingCart(filepath, shopping_cart.quantity.data, shopping_cart.product_name.data,
                                   shopping_cart.product_description.data, shopping_cart.price.data, clinic_id)
    ...
    if 'public_login' in session:
        ...
        if request.method == 'POST':
            ...
            for clinic_id in shopping_db:
                for items in shopping_db[clinic_id]:
                    shopping_list.append(items)
            print('HII')
            counting=quantity_update.quantities.data
            for items in shopping_list:
                ...
                print('')
                print(items)
                items.set_cart_total(shopping_list[0].get_price() * counting)
                print(cart_pricing)
                print(count)
                print(counting)
                ...
                # clinic_id = items.get_clinic_id()
            # print(shopping_list)
            for clinic_id in shopping_db:
                new_shopping_list = []
                for items in shopping_list:
                    if clinic_id == items.get_clinic_id():
                        new_shopping_list.append(items)
                shopping_db[clinic_id] = new_shopping_list
            shopping_db.close()
            if quantity_update.quantities.data<=10:
                return redirect(url_for('checkout'))
            else:
                return render_template('invalid_quantity.html')
        else:
            try:
                shopping_db = shelve.open('shopping.db', 'r')
            except:
...
def checkout():
    if 'public_login' in session:
        ...
        return render_template('ShoppingCheckout.html',subtotal=subtotal,total=total)
    else:
        return redirect(url_for('login_public'))
    ...
    if 'public_login' in session:
        ...
        for clinic_id in shopping_db:
            for items in shopping_db[clinic_id]:
                shopping_list.append(items)
        for items in shopping_list:
            count = str(items.get_cart_total()).replace('.', '') + '0'
            print(count)
...
def create_consultation():
    if 'public_login' in session:
        ...
        if request.method == 'POST' and create_form.validate():
            ...
            try:
                bookings_dict[nric].append(booking)
            except:
                bookings_dict[nric] = []
                bookings_dict[nric].append(booking)
